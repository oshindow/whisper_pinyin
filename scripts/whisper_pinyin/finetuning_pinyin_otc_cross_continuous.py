...
class WhisperModelModule(LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        time_mask_indices = random.sample(range(M + 1, audio_features.shape[1] - M - 1), int(audio_features.shape[1] * self.cfg.index_ratio))

        audio_features_pos = quant_target[:, time_mask_indices]  # (2 * N, C, T) -> (2 * N, T, C)
        neg_indices = [
            random.sample([i for i in range(T) if i != t], M)
            for t in time_mask_indices
        ]
        audio_features_neg = quant_target[:, neg_indices]  # (2 * N, C, T) -> (2 * N, T, C)
        
        audio_features = torch.roll(audio_features, batch_size // 2, dims=0)
        target_encoder_out = audio_features[:,time_mask_indices]
        
        B, K, C = audio_features_pos.shape
        audio_features_neg = audio_features_neg.view(B, K, M, C)
        
        similarity_pos = torch.exp(self.cos(target_encoder_out, audio_features_pos) / 0.1)
        similarity_neg = torch.exp(
            self.cos(
                target_encoder_out.unsqueeze(2),   # (2N, K, 1, C)
