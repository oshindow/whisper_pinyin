...
EXP_DIR="/data2/xintong/checkpoints/qwen3_asr_1_7b_ctc_actual_twostage"
...
if [[ "${FORCE_SPLIT:-0}" == "1" || ! -s "$SPLIT_DIR/train.jsonl" || ! -s "$SPLIT_DIR/dev.jsonl" ]]; then
  ...
  echo "[stage 1/2] Reusing existing split in $SPLIT_DIR."
fi

echo "[stage 2/2] Loading Qwen3-ASR-1.7B audio encoder and starting DDP training..."
...
"$PYTHON_BIN" scripts/baseline/finetuning_pinyin_ctc_qwen3.py \
  ...
  --model-name Qwen/Qwen3-ASR-1.7B-hf \
  --ctc-vocab 188 \
  --exp-dir "$EXP_DIR" \
  --batch-size 4 \
