...
OUTPUT_DIR="${OUTPUT_DIR:-results/qwen3_finalbucket_step29007/tone_tsne_l2_top4_finals_200_per_tone}"
...
"$PYTHON_BIN" scripts/analyze_qwen3_tone_latents.py \
  ...
  --finals i e u ian --samples-per-tone 200 --candidate-multiplier 2 \
  --batch-size 16 --num-workers 0 --seed 42 \
  --output-dir "$OUTPUT_DIR" "$@"

exec "$PYTHON_BIN" scripts/plot_qwen3_hierarchical_tsne.py --input-dir "$OUTPUT_DIR"
